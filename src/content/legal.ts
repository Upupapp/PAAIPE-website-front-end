...
export const PRIVACY_DRAFT: readonly LegalSection[] = [
  ...
  },
  {
    ...
    body: [
      ...
      'When event registration is enabled, it will collect your email address; a record of whether you ticked the box asking for updates, and only if you ticked it; and the version of this notice you were shown, which is the evidence of what you were told at the time. Nothing else. The basis is Section 12(b) of the Act - steps taken at your own request - so that PAAIPE can register you, confirm it, and send you what you need in order to attend the event you asked about - joining details, and any change to them. How many messages that takes, and how far ahead, is not yet decided, so this notice does not promise a number.',
      ...
      'When you unsubscribe, PAAIPE will delete your address and keep only a one-way cryptographic fingerprint of it, for the single purpose of never contacting you again. Your address cannot be read back from that fingerprint - but the fingerprint is still information about you, not anonymous data, and PAAIPE treats it that way. What is gone is the reversible copy: an address that cannot be decrypted cannot be emailed, exported, or read out of a backup. Keeping it is how an unsubscribe survives a future list import that would otherwise add you back with nobody able to tell.',
